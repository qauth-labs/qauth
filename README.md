...
- **Open:** the real-wallet interoperability pass (#376) and the HAIP same-device return leg (#405). HAIP profile wiring #377 closed 2026-09-11 with Phase C (encrypted `direct_post.jwt`), so `haip-1.0` now boots when provisioned; the tracking epic #231 closed 2026-09-10 with its acceptance criteria met against `oid4vp-1.0-base`. Every HAIP result so far is against the in-repo mock wallet.
