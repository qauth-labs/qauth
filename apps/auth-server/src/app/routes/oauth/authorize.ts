    ...
    async (request, reply) => {
      ...
      const createCode = async (): Promise<string> => {
        ...
        await fastify.repositories.authorizationCodes.create({
          ...
          //
          // Read from the raw URL, not from the Zod-parsed `query` object.
          // fastify-type-provider-zod@6.1.0 strips fields whose value fails
          // to match any union branch cleanly AND whose shape changes under
          // `.transform()` — the resource param from Fastify's querystring
          // parser (single value → bare string) does not survive that path.
