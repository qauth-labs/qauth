---
title: Code Examples
description: Copy-paste-ready Node/TypeScript and browser clients for QAuth.
sidebar:
  order: 5
...
environments. See the [API Reference](/integrate/api-reference/) for full endpoint
contracts and the [OAuth 2.1 Flow](/integrate/oauth-flow/) for the protocol details.
...
Mirrors the [first-party auth endpoints](/integrate/api-reference/#first-party-authentication):
...
[MCP Quickstart](/integrate/mcp-quickstart/#option-b--verify-the-handshake-with-curl-no-browser)).
...
(see the [OAuth Flow, step 0](/integrate/oauth-flow/#0-prerequisites--a-client)).
...
> [`refresh_token` grant](/integrate/oauth-flow/#refresh-token-rotation) over persisting
> long-lived tokens.
