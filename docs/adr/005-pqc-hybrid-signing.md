...
>
> **Correction (2026-07-31): the "no ML-DSA-signed token yet" clause above, and the identical clause in the #243 amendment below, are superseded.** #245–#247 landed the JWS carrier, mixed `AKP`+`OKP` JWKS and the introspection-first posture, and **#275 wired live hybrid issuance into the auth-server** — `apps/auth-server/src/app/helpers/hybrid-token.ts` (`issueAccessToken`) is called from `/oauth/token`, `/auth/login` and `/oauth/introspect`. A deployment that sets `SIGNING_ALGORITHM_MODE=ed25519+ml-dsa-65` and `HYBRID_SIGNING_ENABLED=true` **does** emit ML-DSA-65-signed material today. What remains true: both flags default **off**, so a default deployment is still Ed25519-only, and turning hybrid on by default is gated on the pre-default-on checklist in the [security gate review](../security/005-pqc-hybrid-signing-review.md).
>
> **Correction (2026-08-08): the call sites named in the 2026-07-31 note above are slightly wrong.** `issueAccessToken` is imported and called only by `/oauth/token` (`routes/oauth/token.ts`) and `/auth/login` (`routes/auth/login.ts`). `/oauth/introspect` does not call it — it imports `getPqcSignature` from the same helper and uses it to attach the detached `pqc_signature` / `pqc_alg` members to the introspection response. The substance of that note is unaffected: hybrid issuance is live, and introspection is the delivery channel for the PQC component under the default `reference` posture. Only the named call site was inaccurate.
>
...
- [RFC 9964 — ML-DSA for JOSE and COSE](https://www.rfc-editor.org/rfc/rfc9964.html) (published successor to `draft-ietf-cose-dilithium`; governs the `AKP` JWK shape and the `ML-DSA-65` alg spelling QAuth emits)
- [RFC 9864 — Fully-Specified Algorithms for JOSE and COSE](https://www.rfc-editor.org/rfc/rfc9864.html) (alg-identifier policy)
- [IETF draft-ietf-jose-pq-composite-sigs](https://datatracker.ietf.org/doc/draft-ietf-jose-pq-composite-sigs/) (WG-adopted successor to `draft-prabel-…`; the composite construction QAuth does NOT implement)
