...
**Date:** 2026-07-20 · **anchors re-verified against `main` 2026-07-25**
...
**Related work:** gap analysis #286 · RS256 ID-token signing #309 · POST authorize/userinfo + `auth_time` #310 · login-bounce rework #319 · `prompt=none` #318

> **Re-verification note (2026-07-25).** Every `file:line` anchor below was re-checked against `main` after the `release/260725` merge. `authorize.ts` and `schemas/oauth.ts` shifted substantially (#319 rewrote the login-bounce path) and their anchors are corrected throughout. Three changes landed after the first draft and are folded in: **#319** adds a Redis-backed, single-use, 10-minute pending-authorization stash on the unauthenticated authorize path (new **Prerequisite 4** — read it before starting a run), **#318** makes `prompt=none` return `login_required`, and the **`request_uri` metadata defect flagged in Step 7 has been fixed** rather than left as a run-time risk.
...
2. **#309 (RS256) and #310 (POST `/authorize`, POST `/userinfo`, `auth_time`) merged.** Both are on `main` as of 2026-07-20 (#310 via PR #312). They satisfy the suite's `id_token`-signature, POST-endpoint, and `max_age` / `auth_time` modules. **Deploy a build from `main` at or after these merges** — an earlier build signs EdDSA-only, lacks POST `/authorize` and POST `/userinfo`, and emits no `auth_time`, each a hard-fail.
...
4. **Redis reachable, and the login bounce understood (#319, landed 2026-07-25 — after this runbook's first draft).** The unauthenticated `/oauth/authorize` path no longer nests the whole authorize query inside `return_to`. It parks the authorize URL in Redis under a 43-character CSPRNG handle and redirects to `/ui/login?return_to=%2Fui%2Fresume%2F<handle>`; after sign-in, `/ui/resume/:handle` swaps it back (`apps/auth-server/src/app/helpers/pending-authorization.ts`). Three consequences for a certification run:
   - **The stash is single-use and expires after 10 minutes** (`PENDING_AUTHORIZATION_TTL_MS`, `apps/auth-server/src/app/constants/security.ts:101`). Suite modules pause on **Browser Interaction → Visit**; if you leave one parked past ten minutes before completing login, the handle is gone and you land on the "expired" page instead of the authorization request. That reads as an `INTERRUPTED` module, not a QAuth defect — **re-run the module rather than debugging the server.** Likewise, replaying a login URL from browser history will not work: the handle is burned on read.
   - **Redis is on the pre-authentication path — and already was before #319.** The Redis-backed `@fastify/rate-limit` plugin is registered app-wide (`apps/auth-server/src/app/plugins/rate-limit.ts:24-32`) at `apps/auth-server/src/app/app.ts:255`, ahead of both AutoLoad sweeps (lines 294 and 301), and `RATE_LIMIT_ENABLED` defaults to `true` (`libs/server/config/src/lib/schemas/rate-limit.ts:12-15`) — so unauthenticated requests already reached Redis before this change existed. What #319 adds is the first _caller-influenced_ pre-auth **write**. That write degrades rather than 500s — a failed stash falls back to the old inline `return_to` (`apps/auth-server/src/app/helpers/pending-authorization.ts:299`), and a failed read renders the "expired" page (`pending-authorization.ts:245`) — but the fallback reintroduces the multi-kilobyte `Location` header the change exists to avoid, which a reverse proxy in front of the deployment will reject with a 414/400 before the login page renders. And it only covers a stash operation failing on its own: `skipOnError` is left at its default `false`, so an instance-wide Redis outage fails the request at the rate-limiter's `onRequest` hook, before any route handler and therefore before any fallback runs. Confirm Redis is actually reachable from the deployment; do not rely on the fallback.
   - **A `state` over 16 KB is now a `400 invalid_request`** at the authorization endpoint (`PENDING_AUTHORIZATION_MAX_URL_BYTES`, `security.ts:131`), rather than an oversized redirect. Well below anything the suite sends, but worth knowing if you hand-test with a pathological `state`.
...
> **Resolved 2026-07-25 (#286):** the RS256 vars are now documented in `.env.example` alongside the EdDSA block, including the PKCS#1-vs-PKCS#8 pitfall and the `openssl genpkey` recipe. Earlier readings of this runbook noted them as schema-supported but undocumented.
...
`/oauth/authorize` requires an authenticated end-user — a `__Host-qauth_session` cookie or a legacy Bearer (`authorize.ts:275-283`) — so a full round trip needs a test user logged in at `$ISSUER/ui/login` first.

```bash
# 1. PKCE pair (S256 is hard-required — oauth.ts:37)
...
- Accepted fields (unknown keys stripped, `schema:307`; schema at `:309`): `redirect_uris` (`:311`), `grant_types` ⊆ `authorization_code|refresh_token|client_credentials` (`:312-315`), `response_types` = only `code` (`:316-319`), `token_endpoint_auth_method` ∈ `none|client_secret_basic|client_secret_post` (`:320-322`), `scope` (`:323`).
...
- **Redirect-URI exact match** at authorize time — no wildcards (`authorize.ts:169`, `client.redirectUris.includes(redirectUri)`). Register the callback **verbatim**.
- **Environment gate:** DCR clients are created `environment: 'production'` (`register.ts:142`), which permits **HTTPS redirect URIs only**; plain-HTTP `localhost` is rejected outside a `development` client (`authorize.ts:183-207`). The hosted suite callback is HTTPS, so this is fine.
...
3. For each module press **Run Test**. Interactive modules show **Browser Interaction → Visit** — complete QAuth's login + consent, then return to results. Some modules run the flow twice (re-consent / `prompt` / `max_age`) — repeat the login when asked. **Complete each interaction within 10 minutes of it appearing**: per Prerequisite 4, the parked authorization request expires after that and the module ends up `INTERRUPTED` for reasons unrelated to conformance.
...
- [ ] **`auth_time` present** when `max_age` is requested (emitted since #310, now in `main`).
- [ ] **`prompt=none` while unauthenticated → expect `error=login_required` on the `redirect_uri`, not a login page (#318, landed 2026-07-25 — after this runbook's first draft).** OIDC Core §3.1.2.1 forbids any user-facing UI for `prompt=none`; QAuth now returns the error on the client's redirect URI instead of bouncing to `/ui/login` (`authorize.ts:284-292`, and the same check on the authenticated step-up path below it). Before #318 a silent-renewal request rendered a login form inside the client's hidden iframe and hung until timeout. If a module exercises `prompt=none`, this is the behaviour it should see.
...
- [ ] **Optional `address` / `phone` scopes → expect SKIP/WARN.** `scopes_supported` = `['openid','profile','email','offline_access']` (`discovery.ts:19-24`); unsupported scopes are silently dropped (`filterRequestedScopes`, `authorize.ts:409`). No `address`/`phone` claims are ever returned. Nothing to fix.
- [x] **`request` / `request_uri` modules → expect SKIP/PASS. RESOLVED 2026-07-25 (#286), was the flagged risk.** `authorizeQuerySchema` is a plain `z.object` with default-strip and has no `request`/`request_uri` field (`schemas/oauth.ts:28-56`), so those params are silently stripped and QAuth does **not** return `request_not_supported` / `request_uri_not_supported`. The metadata used to advertise neither flag, which was an actual defect rather than merely a suite risk: RFC 8414 §2 and OIDC Discovery §3 default an omitted `request_parameter_supported` to `false` (matched reality) but an omitted `request_uri_parameter_supported` to **`true`** (did **not** — QAuth ignores `request_uri`), so the published document claimed a capability the server does not have. Both are now stated explicitly as `false` in `buildAuthorizationServerMetadata`, so **both** the OIDC and RFC 8414 documents carry them. Guarded by three tests, including one that parses a `request_uri`-bearing authorize query through the real schema so the advertised `false` cannot go stale if request-object handling is ever added.
- [ ] **`claims` request parameter (OIDCCClaimsEssential) → expect SKIP.** No `claims` field in `authorizeQuerySchema` (`oauth.ts:28-56`); discovery does not advertise `claims_parameter_supported` (defaults `false`, which matches reality — unlike the `request_uri` case above). Basic OP does not require it.
- [ ] **Authorization-code reuse → expect PASS (`invalid_grant`).** Codes are single-use: `findByCode` returns only unused, unexpired codes (`libs/infra/db/src/lib/repositories/authorization-codes.repository.ts:43, 53`), and a successful exchange atomically flips `used` (`markUsed`, `:72-82`, called `token.ts:443`); reuse → `undefined` → `invalid_grant` (`token.ts:306-318`). **Gap to note:** QAuth does **not** additionally revoke tokens previously issued from the reused code (RFC 9700 §4.5 SHOULD) — generally not a Basic OP hard-fail, but flag it if a module checks token revocation on replay.
...
| `max_age` / `auth_time`                                  | `auth_time` emitted (epoch seconds) when `max_age` is requested                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | #310 (in `main`)                                                    | PASS        |
| `prompt=none`, unauthenticated                           | `error=login_required` on the `redirect_uri`; no login UI (#318)                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `authorize.ts:284-292`                                              | PASS        |
| UserInfo (`GET`/`POST`)                                  | Plain JSON; `email`/`email_verified` only when a verified email exists (#229)                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `discovery.ts:173-175`                                              | PASS        |
| Authorization-code reuse rejected                        | Single-use, atomic `markUsed`, reuse → `invalid_grant`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | `authorization-codes.repository.ts:43,53,72-82`; `token.ts:306-318` | PASS        |
| `scope=address` / `scope=phone`                          | Unsupported; silently dropped                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `discovery.ts:19-24`; `authorize.ts:409`                            | SKIP/WARN   |
| `request` / `request_uri` object modules                 | Silently stripped; not rejected. Both flags now advertised `false` (#286)                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `oauth.ts:28-56`; `discovery.ts:121-133`                            | SKIP/PASS   |
| `claims` request parameter                               | Silently stripped; `claims_parameter_supported` defaults false                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | `oauth.ts:28-56`; `discovery.ts:149-177`                            | SKIP        |
| Config OP (discovery-only)                               | Same discovery doc; `jwks_uri` resolves                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | `discovery.ts:68-177`                                               | PASS        |
