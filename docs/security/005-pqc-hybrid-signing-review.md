...
| --- | ------ | ------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------ |
| F1  | MEDIUM | ~~Downgrade decision reads only the _unsigned_ transport `pqcSignature`~~ — **RESOLVED in #275**                               | `hybrid-signing.ts`                                    |
...
| F5  | LOW    | ~~ML-DSA key must be resolved from the _signed_ `pqc_kid` header~~ — **RESOLVED in #275**                                      | `hybrid-signing.ts`                                    |
...
- **F3 (wrong draft pin).** `PQC_JOSE_COMPOSITE_DRAFT = 'draft-prabel-jose-pq-composite-sigs-02'` is designated as the draft-churn firewall, but `kty:'AKP'` / `pub` / the `ML-DSA-65` alg spelling actually come from **`draft-ietf-cose-dilithium`** ("ML-DSA for JOSE and COSE"); the composite-sigs draft describes the _concatenated composite_ QAuth deliberately does **not** implement, and the individual `-prabel-…-02` pin is stale versus the WG-adopted `draft-ietf-jose-pq-composite-sigs`. The **emitted wire shape happens to match cose-dilithium, so there is no live interop break** — but the safeguard meant to catch draft churn points a future reviewer at the wrong document. Tracked as checklist item 1.
  **RESOLVED (#274).** The constant was replaced by `PQC_JOSE_MLDSA_SPEC = 'RFC 9964'` — during remediation the cose-dilithium draft was found to have been **published as RFC 9964** (Standards Track, May 2026; final draft revision `-11`), so the AKP shape and `ML-DSA-65` spelling are now pinned to a stable, IANA-registered standard rather than a draft. `PQC_JOSE_ALG_POLICY_SPEC = 'RFC 9864'` records the fully-specified-algorithm policy. The misleading composite constant was **removed** (nothing referenced it; QAuth implements no composite path). The emitted `AkpJwk` was re-verified against RFC 9964: `kty`/`alg`/`pub` REQUIRED and present, `priv` absent, and a test now asserts the **exact** member set.
...
1. ~~**Correct the JOSE pin [F3].**~~ **DONE (#274).** Pinned to `PQC_JOSE_MLDSA_SPEC = 'RFC 9964'` (the published cose-dilithium successor) and `PQC_JOSE_ALG_POLICY_SPEC = 'RFC 9864'`; the composite-sigs constant was removed (re-add under its own name, citing `draft-ietf-jose-pq-composite-sigs`, only if a composite path is later added). The emitted `AkpJwk` was diffed against the RFC 9964 example JWK and matches, with a test asserting the exact member set. **Still open:** confirming an _independent_ ML-DSA-in-JOSE verifier resolves `/.well-known/jwks.json` — this needs a third-party verifier and a running server, so it remains a pre-default-on manual step.
2. ~~**Wire hybrid issuance + a PQC-aware verify path** into the live auth-server routes (#247 delivery, #249 introspection/mcp-guard).~~ **DONE (#275).** The token endpoint issues hybrid tokens when `HYBRID_SIGNING_ENABLED` is on; the detached ML-DSA-65 signature is stored per-`jti` and returned from `/oauth/introspect` as `pqc_signature` (reference delivery). `jwtUtils.verifyHybridAccessToken` is the PQC-aware verify path.
3. ~~**Make downgrade resistance issuer-signed [F1].**~~ **DONE (#275).** `verifyWithHeader()` surfaces the Ed25519-verified protected header (`signing.ts`); `verifyHybrid` treats a present signed `pqc_alg` as BINDING regardless of `requirePqc` and cross-checks `hybrid.pqcAlg` against it. A token with no signed `pqc_alg` is rejected under `requirePqc=true`, and any unvouched-for attached signature is ignored. Covered by the strip-with-`requirePqc=false` rejection test in `hybrid-signing.test.ts` and end-to-end in `hybrid-token.test.ts`.
4. ~~**Bind ML-DSA key resolution to the signed `pqc_kid` [F5]**~~ **DONE (#275).** `HybridVerifyKey.mlDsa` accepts an `MlDsaKeyResolver` invoked with the SIGNED `pqc_kid` read from the verified header, failing closed when nothing resolves. The unsigned `HybridSignedToken.pqcKid` transport field was **dropped** — nothing depended on it.
...
8. **Stand up a trustworthy native-build channel [F4]** before enabling the native backend anywhere: reproducible CI build of the `.node` from pinned source, published checksum + provenance attestation, load-time integrity verification, and `cargo audit` + `cargo deny`/`vet` in CI for `crypto-native`.
