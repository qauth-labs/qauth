...
where a presentation becomes a session or a linked credential. Three gates run,
...
3. **#300 — subject resolution.** ADR-009 §1 requires verifying that the presented
...
Note the boot posture, deliberately unchanged by #238: selecting a strategy is
**not** a startup gate. A deployment that names a strategy and omits its settings
fails at `resolveSubjectResolution` on the request path — logged as an operator
error and rendered as a refusal, never as a distinct status an anonymous caller
could drive.

The login path does **not** enrol. Every non-`matched` outcome, `no-match`
included, becomes the same refusal: ADR-009 §1's first bootstrap case
(establishing an account and its binding together) is a registration decision,
and turning it into an account at the login screen would make that screen an
enrolment oracle.
