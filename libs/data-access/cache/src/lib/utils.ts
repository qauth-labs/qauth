...
export class CacheUtils {
   ...
   * Set cache data
   */
  static async setCache(key: string, data: any, ttl: number = DEFAULT_TTL.CACHE): Promise<void> {
    const client = getRedis();
...
export class UserUtils {
   ...
   * Get user data
   */
  static async getUserData(userId: string): Promise<Record<string, any> | null> {
    const client = getRedis();
    const key = `${KEY_PREFIXES.USER}${userId}`;
    const data = await client.get(key);
    return data ? JSON.parse(data) : null;
