...
describe('prepareWalletLink — the binding is what makes the link worth anything', () => {
  ...
  it('records the VALIDATED issuer and the credential type, not the asserted ones', async () => {
    ...
    expect(outcome.credentialData.credential_type).toBe(credential.credentialType);
    // The write shape is #235's, shared with the enrolment path — a link and a
    // first-time login must produce the same column, or the credential linked
    // today reads as unbound at the next `asserted-lookup` login.
    expect(outcome.credentialData.credential_format).toBe(credential.format);
    expect(outcome.credentialData.subject_resolution).toBe(ASSERTED_LOOKUP.strategy);
  });
