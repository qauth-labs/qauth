...
/**
 * Anchored `x5c` chain validation, shared by every JOSE artifact QAuth verifies
 * through a certificate chain.
 *
 * ## Two callers, one implementation
 *
 * - **Status List Tokens** (#297, HAIP §6.1.1) — `status/status-list-chain.ts`.
 * - **OID4VCI Appendix D key attestations** (#308, HAIP §4.5.1) —
 *   `attestation/key-attestation.ts`.
 *
 * The two artifacts are unrelated, but the rules HAIP states for their chains
 * are word-for-word the same ones: the verification key MUST be in the `x5c`
 * header, the trust anchor MUST NOT be in it, and the signing certificate MUST
 * NOT be self-signed. Duplicating a DER walk and a path-building loop so that
 * each caller could own a copy is exactly how one copy quietly stops enforcing
 * something the other still does, so there is one implementation and the
 * callers differ only in which ANCHOR SET they hand it.
 *
 * The anchor sets are emphatically NOT shared. A status issuer's CA and a wallet
 * provider's CA answer different questions, and an operator who configured one
 * must not find it silently authorising the other — which is why anchors arrive
 * as a parameter and this module owns no configuration of its own.
 *
 ...
 * to be a small, private, purpose-issued set.
 *
 ...
 * signs the artifact directly" case, since a conforming CA certificate carries
 * `keyCertSign`/`cRLSign` without `digitalSignature`.
 *
 ...
 * same name and sign with that. The control that would actually bound a rogue
 * anchored intermediate is name constraints, which is listed above as not
...
/** The only leaf key type an `ES256` signature may be verified with. */
 ...
 */
export interface X509TrustAnchors {
 ...
 */
export const NO_X509_TRUST_ANCHORS: X509TrustAnchors = Object.freeze({
  size: 0,
  certificates: Object.freeze([]),
});

/**
 * How a caller names its anchor set in operator-facing configuration errors.
 *
 * Passed in rather than hard-coded because the two callers configure two
 * DIFFERENT anchor sets from two different environment variables, and an
 * operator who mis-pastes one PEM must be told which of them they broke.
 */
export interface TrustAnchorDescriptor {
  /** Singular, lower case — e.g. `'status list trust anchor'`. */
  readonly noun: string;
  /** The issue that owns the setting — e.g. `'#297'`. */
  readonly issue: string;
}

/**
 * Compile operator-supplied PEM certificates into a trust-anchor set.
 *
 * Anchors are the root of the whole path's authority, so a malformed one throws
 * loudly (an OPERATOR error) rather than being dropped — dropping it would
 * leave the operator believing an issuer is anchored when it is not. The
 ...
 * @param descriptor - how to name this anchor set in a configuration error.
 * @returns a frozen anchor set.
 * @throws InvalidConfigurationError when an entry is not a parseable
 * certificate.
 */
export function createX509TrustAnchors(
  pems: readonly string[],
  descriptor: TrustAnchorDescriptor
): X509TrustAnchors {
  const plural = `${descriptor.noun}s`;

  if (!Array.isArray(pems)) {
    throw new InvalidConfigurationError(
      `${capitalize(plural)} must be an array of PEM-encoded X.509 certificates (${descriptor.issue}).`
    );
  }
  ...
  for (const [index, pem] of pems.entries()) {
    ...
    if (certificate === undefined) {
      throw new InvalidConfigurationError(
        `A ${descriptor.noun} is not a parseable PEM-encoded X.509 certificate (${descriptor.issue}). See this error's "details" for the position and the value.`,
  ...
  if (certificates.length === 0) return NO_X509_TRUST_ANCHORS;

  return Object.freeze({ size: certificates.length, certificates: Object.freeze(certificates) });
}

/** Upper-case the first character of an operator-facing noun phrase. */
...
/** The result of resolving a signing certificate out of an `x5c` header. */
export type ChainResolution =
  | {
      readonly outcome: 'resolved';
      /** The end-entity certificate whose key signed the artifact. */
...
/**
 * Whether `leaf` is authorised to sign a detached JOSE artifact.
 *
 ...
 */
function leafMaySign(leaf: X509Certificate): boolean {
...
/**
 * Resolve a signing key from an `x5c` header, anchored to operator config.
 *
 ...
 * EXCLUDED from it (HAIP §4.5.1 for key attestations, §6.1.1 for Status List
 * Tokens — the same rule, stated twice). An anchor found inside the chain is
 * refused outright rather than tolerated: a self-signed root shipped in `x5c`
 * would otherwise satisfy the path check against itself, which turns "chains to
 * an anchor" into "carries a copy of an anchor".
 *
 * @param x5c - the `x5c` member of the artifact's protected header, unverified.
 * @param anchors - the operator's configured anchors for THIS artifact kind.
 ...
 * throws.
 */
export function resolveAnchoredSigningCertificate(
  x5c: unknown,
  anchors: X509TrustAnchors,
  now: Date
): ChainResolution {
